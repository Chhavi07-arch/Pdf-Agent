...
export default function App() {
  ...
  function handleUploadSuccess(newSessionId, filename, chunkCount) {
    // summary: undefined → still generating; string → ready; null → unavailable
    setUploadedPdfs(prev => [...prev, { sessionId: newSessionId, filename, chunkCount, summary: undefined }])
    setActiveSessionId(newSessionId)
    setMessagesMap(prev => ({ ...prev, [newSessionId]: [] }))
    loadSummary(newSessionId)
  }

  // The document summary is generated by a background task after upload, so we
  // poll the session endpoint until it appears (or give up after a few tries).
  async function loadSummary(sid, tries = 8, intervalMs = 1200) {
        ...
        if (res.ok) {
          ...
          if (data.summary) {
            setUploadedPdfs(prev =>
              prev.map(p => (p.sessionId === sid ? { ...p, summary: data.summary } : p)))
            return
          }
        }
    ...
    // Stop showing "generating": mark unavailable if still nothing.
    setUploadedPdfs(prev =>
      prev.map(p => (p.sessionId === sid && p.summary === undefined ? { ...p, summary: null } : p)))
  }
